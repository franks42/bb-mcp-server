...
(deftest on-connect-callback-test
         (testing "on-connect callbacks are called before initial sync"
      ...
      ;; Register callback that registers an atom just-in-time
                    (let [!late-state (atom {:late true})]
                      (server/register-on-connect!
                       :test-callback
                       (fn [conn-id]
                         (swap! callback-calls conj conn-id)
           ;; Register atom just-in-time during callback
                         (core/register-synced-atom! :late-state !late-state)))

        ;; Simulate browser connect
                      (server/on-browser-connected! "conn-456")

        ;; Callback should have been called
                      (is (= ["conn-456"] @callback-calls))

        ;; Late-registered atom should be in the sends
                      (let [sent-keys (set (map #(get-in % [1 1 :key]) @sends))]
                        (is (contains? sent-keys :late-state)))

        ;; Cleanup
                      (server/unregister-on-connect! :test-callback)))))
